...
namespace Lab4.Interpreting {
	sealed class Interpreter : IStatementVisitor, IExpressionVisitor<object> {
		...
		object CalcAddition(Binary binary) {
			...
			return Calc<BigInteger>(binary.Left) + Calc<BigInteger>(binary.Right);
		}
		...
		object CalcLess(Binary binary) {
			...
			if (a is int && b is BigInteger) {
				return (int)a < (BigInteger)b;
			}
		...
		public object VisitNumber(Number number) {
			if (BigInteger.TryParse(number.Lexeme, NumberStyles.None, NumberFormatInfo.InvariantInfo, out BigInteger value)) {
				return value;
			}
			throw MakeError(number, $"Не удалось преобразовать {number.Lexeme} в Biginteger");
		}
