	...
	sealed class Parser {
		...
		IStatement ParseStatement() {
			...
			if (SkipIf("=")) {
				...
				else if (leftExpression is ElementAccess elementAccess) {
					return new ArrayAssignment(elementAccess, rightExpression);
				}
				else {
					throw MakeError("Присваивание не в переменную или элемент массива");
				}
			}
			else {
				Expect(";");
		...
		IExpression ParsePrimary() {
			...
			while (true) {
				...
				if (SkipIf("(")) {
					...
					expression = new ElementAccess(pos, expression, index);
					Expect("]");
				}
				else {
					break;
		...
		IExpression ParsePrimitive() {
			...
			if (SkipIf("[")) {
				...
				return new ArrayLiteral(pos, arguments);
			}
