...
using Lab4.Interpreting.Values;
...
namespace Lab4.Interpreting {
	sealed class Interpreter : IStatementVisitor, IExpressionVisitor<object> {
		...
		public void VisitArrayAssignment(ArrayAssignment arrayAssignment) {
			var array = Calc<object[]>(arrayAssignment.Left.Obj);
			var index = Calc<int>(arrayAssignment.Left.Index);
			var rightExpr = Calc(arrayAssignment.Expr);
			array[index] = rightExpr;
		}
		#endregion
		#region expressions
		...
		public object VisitArrayCreation(ArrayLiteral arrayCreation) {
			return arrayCreation.Items.Select(Calc).ToArray();
		}
		public object VisitElementAccess(ElementAccess elementAccess) {
			var array = Calc<object[]>(elementAccess.Obj);
			var index = Calc<int>(elementAccess.Index);
			return array[index];
		}
